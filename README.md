...
.
├── data/                              # テーブル生成の入力データ
├── outputs/                           # 生成されたローマ字テーブル
│   ├── azooKey/
│   │   └── dvorak_jp.tsv              # azooKey 用ローマ字テーブル
│   ├── google_japanese_input/
│   │   ├── dvorak_jp.tsv              # Google 日本語入力用ローマ字テーブル
│   │   └── dvorak_jp_with_emoji.tsv   # 絵文字変換対応版
│   └── emoji.tsv                      # 取得した絵文字データ
└── src/                               # テーブルを生成する Rust CLI のソースコード
```
